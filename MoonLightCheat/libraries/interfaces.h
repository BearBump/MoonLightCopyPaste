...
public:
	...
	void GetInterfaces()
	{
		...
		Client = (CClient*)GetPointer("client_panorama.dll", "VClient");
		ClientEntList = (CClientEntityList*)GetPointer("client_panorama.dll", "VClientEntityList");
		...
		for (DWORD dwIter = 0; dwIter <= 0xFF; dwIter++)
		{
			if (*(PBYTE)(dwInitAddr + dwIter) == 0xA3)
			{
				...
				printf("Globals Found: 0x%X || 0x%X\n", (DWORD)Globals, ((DWORD)Globals - (DWORD)GetModuleHandle("client_panorama.dll")));
				break;
			}
		}
